...
function gameLandingHtml(game) {
  ...
  return `<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${game.title}</title><meta name="description" content="${game.description}"><meta name="robots" content="index,follow,max-image-preview:large,max-snippet:-1"><link rel="canonical" href="${canonical}"><meta property="og:type" content="website"><meta property="og:locale" content="ar_IQ"><meta property="og:title" content="${game.title}"><meta property="og:description" content="${game.description}"><meta property="og:url" content="${canonical}"><script type="application/ld+json">${schema}</script><style>*{box-sizing:border-box}body{margin:0;background:#070706;color:#fff;font-family:Tahoma,Arial,sans-serif;min-height:100vh;display:grid;place-items:center;padding:24px}.box{width:min(820px,100%);background:#11100d;border:1px solid #765623;border-radius:22px;padding:34px;box-shadow:0 24px 70px #000}.crumb{color:#c9ad70;font-size:14px}.crumb a{color:#f2d589}h1{color:#ffe49a;font-size:clamp(28px,5vw,44px);margin:18px 0 12px}p{color:#d0c5ae;line-height:2;font-size:17px}.actions{display:flex;gap:12px;flex-wrap:wrap;margin-top:26px}.btn{display:inline-block;padding:13px 20px;border-radius:12px;text-decoration:none;font-weight:800}.primary{background:#e9b84f;color:#171006}.secondary{border:1px solid #826126;color:#f5dda3}.more{margin-top:28px;padding-top:20px;border-top:1px solid #3c321f;color:#a99b80;font-size:14px}</style></head><body><main class="box"><div class="crumb"><a href="/">مركز ألعاب حماده</a> ← ${game.name}</div><h1>${game.name}</h1><p>${game.description}</p><h2>كيف تستخدم ${game.name} في البث المباشر؟</h2><p>افتح اللعبة من مركز ألعاب حماده ثم ابدأ الجولة أمام جمهورك. يدخل المتابعون أو يتفاعلون حسب نظام اللعبة، وتظهر مشاركاتهم داخل الجولة بصورة مباشرة. صُممت الصفحة لتكون سهلة الاستخدام على شاشة البث، مع إبقاء التحكم الأساسي بيد صاحب البث حتى يستطيع بدء الجولة أو إعادتها ومتابعة النتيجة بدون تعقيد.</p><h2>لماذا تناسب ألعاب المتابعين؟</h2><p>الفكرة الأساسية هي تحويل المشاهد من متفرج إلى مشارك. التعليقات واللايكات والهدايا يمكن أن تصبح جزءًا من اللعب بحسب قواعد كل تحدٍ، وهذا يجعل ${game.name} مناسبة للبثوث التفاعلية والمسابقات الخفيفة. كما يمكنك الرجوع إلى المركز واختيار لعبة أخرى بسرعة إذا أردت تغيير نوع التحدي أثناء اللايف.</p><h2>تشغيل اللعبة</h2><p>استخدم زر تشغيل اللعبة للانتقال إلى النسخة التفاعلية، أو تصفح بقية الألعاب من المركز. وللمزيد راجع دليل <a href="/guide/tiktok-live-games.html">ألعاب تيك توك لايف</a> ودليل <a href="/guide/live-stream-games.html">ألعاب البث المباشر مع المتابعين</a>.</p><h2>نصائح لجولة أفضل</h2><p>قبل بدء ${game.name} اشرح للمتابعين طريقة المشاركة بكلمات قصيرة وواضحة، واترك وقتًا كافيًا للانضمام قبل تشغيل الجولة. عند انتهاء الجولة أعلن الفائز وابدأ تحديًا جديدًا أو انتقل إلى لعبة أخرى من المركز. هذا الأسلوب يساعد الجمهور على فهم القواعد بسرعة ويحافظ على استمرار التفاعل داخل البث.</p><h2>أسئلة شائعة عن ${game.name}</h2><h3>هل يحتاج المتابع إلى تثبيت برنامج؟</h3><p>المتابع لا يحتاج إلى تثبيت اللعبة على جهازه حتى يشارك في الجولة؛ المشاركة تتم من خلال تفاعله مع البث حسب القواعد التي يحددها صاحب اللايف. صاحب البث فقط يفتح مركز ألعاب حماده ويشغّل اللعبة المناسبة.</p><h3>هل يمكن تغيير اللعبة أثناء اللايف؟</h3><p>نعم. بعد انتهاء الجولة تستطيع الرجوع إلى مركز الألعاب واختيار تحدٍ آخر من القائمة، مثل المحيبس أو الكراسي أو المتاهة أو التخمين أو السباقات. بهذه الطريقة يمكن تنويع البث والمحافظة على تفاعل الجمهور بدل تكرار نفس الجولة طوال الوقت.</p><div class="actions"><a class="btn primary" href="/games/${game.file}">تشغيل اللعبة</a><a class="btn secondary" href="/">عرض جميع الألعاب</a></div><div class="more">ألعاب تيك توك لايف • ألعاب للمتابعين • ألعاب تفاعلية للبث المباشر</div></main></body></html>`;
}
...
function prepareCenterHtml(data) {
  ...
  html = html.replace(/<article class="card([^"]*)" onclick="openGame\('([^']+)'\)">/g, (m, extra, name) => {
    ...
    if (!game) return m;
    return '<a class="card' + extra + '" href="/play/' + game.slug + '" onclick="event.preventDefault();openGame(\'' + name + '\')">';
  });
  html = html.replace(/<\/article>/g, '</a>');
  html = html.replace('</head>', '<style>.card{text-decoration:none;color:inherit}</style></head>');
  const listSchema = JSON.stringify({'@context':'https://schema.org','@type':'ItemList',name:'ألعاب مركز ألعاب حماده',itemListElement:GAME_SEO.map((g,i)=>({'@type':'ListItem',position:i+1,name:g.name,url:SITE_ORIGIN+'/play/'+g.slug}))}).replace(/</g,'\\u003c');
  html = html.replace('</head>', '<script type="application/ld+json">' + listSchema + '</script></head>');
  return html;
}
...
const server = http.createServer((req, res) => {
  ...
  if (url === '/health') {
    ...
    res.end(JSON.stringify({ ok:true, liveProvider:'tikfinity-local', revision:'tikfinity-center-srcdoc-v3' }));
    return;
  }
  res.writeHead(404, {'Content-Type':'text/plain; charset=utf-8'}); res.end('Not found');
});

server.on('upgrade', (req, socket) => { socket.end('HTTP/1.1 410 Gone\r\nConnection: close\r\n\r\n'); });
